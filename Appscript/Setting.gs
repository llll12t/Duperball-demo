...
function doGet(e) {
  ...
  if (action === 'fetchHolidays') return fetchHolidays(branch);               // ตั้งค่า - วันหยุด (legacy)
  if (action === 'fetchWorkdays') return fetchHolidays(branch);               // ตั้งค่า - วันทำงานพิเศษ
  ...
  if (action === 'fetchBotStatus') return fetchBotStatus();                   // Bot Status
...
function doPost(e) {
  ...
  else if (req.action === 'bulkUpdateTimeSlots') msg = bulkUpdateTimeSlots(req.data, req.branch);
  else if (req.action === 'deleteTimeSlot') msg = deleteTimeSlot(req.id, req.branch);
    else if (req.action === 'addHoliday') msg = addHoliday(req.date, req.branch); // legacy
    else if (req.action === 'deleteHoliday') msg = deleteHoliday(req.date, req.branch); // legacy
    else if (req.action === 'updatePermanentHolidays') msg = updatePermanentHolidays(req.data, req.branch); // legacy
    else if (req.action === 'addWorkday') msg = addHoliday(req.date, req.branch);
    else if (req.action === 'deleteWorkday') msg = deleteHoliday(req.date, req.branch);
    else if (req.action === 'updateWeeklyWorkdays') msg = updatePermanentHolidays(req.data, req.branch);
  else if (req.action === 'toggleBotStatus') msg = toggleBotStatus(); // เก็บไว้สำหรับ backward compatibility
  else if (req.action === 'updateBotStatus') msg = updateBotStatus(); // ฟังก์ชันใหม่
...
// รองรับ bulk update ช่วงเวลา
function bulkUpdateTimeSlots(rows, branch) {
  branch = branch || '1';
  ...
  rows.forEach(row => {
    ...
    for (let i = 1; i < values.length; i++) {
      if (String(values[i][0]) === String(id)) {
        // อัปเดตเฉพาะช่วงเวลาและสลอต
        sheet.getRange(i + 1, 2, 1, 2).setValues([[row['ช่วงเวลา'] || '', row['สลอต'] || '']]);
        updatedCount++;
        found = true;
        break;
      }
    }
    // ถ้าไม่เจอ id ให้เพิ่มใหม่
    if (!found) {
      id = String(sheet.getLastRow() + 1);
      sheet.appendRow([id, row['ช่วงเวลา'] || '', row['สลอต'] || '', '']);
      updatedCount++;
    }
  });
  return { success: true, message: `อัปเดตข้อมูลช่วงเวลาทั้งหมดแล้ว (${updatedCount} รายการ)` };
}
...
// === Bot Execute Management ===
function fetchBotStatus() {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(SHEET_BOT_EXECUTE);
    if (!sheet) {
      return ContentService.createTextOutput(JSON.stringify({
        success: false,
        message: 'ไม่พบชีต bot_execute'
      })).setMimeType(ContentService.MimeType.JSON);
    }

    // อ่านข้อมูลจากแถว 2 (เลิก header)
    // โครงสร้าง: A: status | B: lasted execute | C: stage
    const status = sheet.getRange(2, 1).getValue(); // A2
    let lastedExecute = sheet.getRange(2, 2).getValue(); // B2
    const stage = sheet.getRange(2, 3).getValue(); // C2

    // แปลง Date object เป็น string format dd/MM/yyyy, HH:mm:ss
    if (lastedExecute instanceof Date) {
      lastedExecute = Utilities.formatDate(lastedExecute, 'Asia/Bangkok', 'dd/MM/yyyy, HH:mm:ss');
    }

    return ContentService.createTextOutput(JSON.stringify({
      success: true,
      status: status,
      lastedExecute: lastedExecute,
      stage: stage
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
...
// ฟังก์ชันสำหรับเซ็ต status เป็น TRUE เพื่อให้ระบบอีกตัวทำงาน
function updateBotStatus() {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(SHEET_BOT_EXECUTE);
    if (!sheet) {
      return { success: false, message: 'ไม่พบชีต bot_execute' };
    }

    // เซ็ต status เป็น TRUE
    sheet.getRange(2, 1).setValue('TRUE'); // A2: status
    
    // อัพเดทเวลา lasted execute
    const now = Utilities.formatDate(new Date(), 'Asia/Bangkok', 'dd/MM/yyyy, HH:mm:ss');
    sheet.getRange(2, 2).setValue(now); // B2: lasted execute
    
    // เซ็ต stage เป็น pending (รอระบบอีกตัวทำงาน)
    sheet.getRange(2, 3).setValue('pending'); // C2: stage

    return {
      success: true,
      message: 'เริ่มกระบวนการอัพเดทข้อมูลแล้ว กรุณารอสักครู่...',
      status: 'TRUE',
      lastedExecute: now,
      stage: 'pending'
    };
...
// ฟังก์ชัน toggleBotStatus เก่า (เก็บไว้สำหรับ backward compatibility)
function toggleBotStatus() {
  try {
    ...
    // อ่านค่า status ปัจจุบัน
    const currentStatus = sheet.getRange(2, 1).getValue(); // A2
    
    // สลับค่า (TRUE <-> FALSE)
    const newStatus = currentStatus === true || String(currentStatus).toUpperCase() === 'TRUE' ? 'FALSE' : 'TRUE';
    
    // อัพเดทวันที่เวลา
    const now = Utilities.formatDate(new Date(), 'Asia/Bangkok', 'dd/MM/yyyy, HH:mm:ss');
    
    // บันทึก status และ lasted execute
    sheet.getRange(2, 1).setValue(newStatus); // A2: status
    sheet.getRange(2, 2).setValue(now); // B2: lasted execute
    // C2 (stage) ไม่แก้ไข เก็บค่าเดิม

    return {
      success: true,
      message: `สลับสถานะเป็น ${newStatus} เรียบร้อย`,
      newStatus: newStatus,
      lastedExecute: now
    };
  } catch (error) {
